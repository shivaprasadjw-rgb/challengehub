...
export async function POST(
  ...
  try {
    ...
    if (action === 'checkCompletion') {
      const result = await checkAndUpdateTournamentStatus(params.tid);
      ...
      if (result) {
        ...
        await appendAudit({
          ...
          details: { message: 'Tournament completion status checked and updated' },
          tournamentId: params.tid
        });

        return NextResponse.json({
          success: true,
          message: 'Tournament status checked and updated successfully'
        });
      } else {
        return NextResponse.json({
          success: false,
