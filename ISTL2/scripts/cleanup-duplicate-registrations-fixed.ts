...
async function cleanupDuplicateRegistrationsFixed() {
  try {
    ...
    const allRegistrations = await prisma.registration.findMany({
      ...
      orderBy: { id: 'asc' } // Use ID for ordering instead of createdAt
    });
    ...
    // Keep only the first 32 registrations (oldest by ID)
...
cleanupDuplicateRegistrationsFixed();